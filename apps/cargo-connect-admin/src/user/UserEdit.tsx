...
import { TokenWalletTitle } from "../tokenWallet/TokenWalletTitle";
import { RatingTitle } from "../rating/RatingTitle";
import { FeedbackTitle } from "../feedback/FeedbackTitle";
import { AffiliateTitle } from "../affiliate/AffiliateTitle";
  ...
  return (
    <Edit {...props}>
      <SimpleForm>
        ...
        <ReferenceArrayInput
          source="tokenWallets"
          reference="TokenWallet"
          parse={(value: any) => value && value.map((v: any) => ({ id: v }))}
          format={(value: any) => value && value.map((v: any) => v.id)}
        >
          <SelectArrayInput optionText={TokenWalletTitle} />
        </ReferenceArrayInput>
        <ReferenceArrayInput
          source="ratings"
          reference="Rating"
        ...
        <ReferenceArrayInput
          ...
          format={(value: any) => value && value.map((v: any) => v.id)}
        >
          <SelectArrayInput optionText={AffiliateTitle} />
        </ReferenceArrayInput>
      </SimpleForm>
    </Edit>
  );
