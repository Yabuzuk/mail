/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AffiliateWhereInput } from "./AffiliateWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AffiliateOrderByInput } from "./AffiliateOrderByInput";

@ArgsType()
class AffiliateFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AffiliateWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AffiliateWhereInput, { nullable: true })
  @Type(() => AffiliateWhereInput)
  where?: AffiliateWhereInput;

  @ApiProperty({
    required: false,
    type: [AffiliateOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AffiliateOrderByInput], { nullable: true })
  @Type(() => AffiliateOrderByInput)
  orderBy?: Array<AffiliateOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AffiliateFindManyArgs as AffiliateFindManyArgs };
